#!/usr/bin/env python3
"""
US-Iran Digest Hourly Check Script - 按照SKILL.md标准每小时执行
这个脚本会在整点后检查并生成对应时段的digest
"""
...
def check_new_developments():
    ...
    # 检查当前时间，如果是凌晨时段，检查是否有夜间活动
    current_time = datetime.now(timezone.utc)
    target_hour = (current_time.hour - 1) % 24
    
    # 模拟检查新进展 - 在实际应用中这里应该检查实时新闻源
    has_new_developments = True  # 默认认为有新进展
    
    print(f"检查时段: {target_hour:02d}:00-{target_hour+1:02d}:00")
...
def commit_to_github(filepath):
    ...
    try:
        ...
        # 提交
        commit_msg = f"Add digest for {target_date}T{target_hour:02d}"
...
def quality_check_existing_file(filepath):
    """对现有文件进行质量检查"""
    print(f"=== 对现有文件进行质量检查: {filepath} ===")
    
    try:
        # 使用现有的audit_digest.py进行检查
        result = subprocess.run([
            'python3', 'skills/usiran-digest-check/audit_digest.py', filepath
        ...
        if result.returncode == 0:
            ...
            print(f"❌ 质量检查失败: {result.stderr}")
            return False
    except Exception as e:
        print(f"❌ 质量检查异常: {e}")
        return False

def execute_hourly_digest_check():
    """执行每小时digest检查"""
    print("=== US-Iran Digest 每小时检查和生成 ===")
    ...
    if existing_file:
        ...
        # 对现有文件进行质量检查
        if quality_check_existing_file(existing_file):
            print("✅ 现有文件质量检查通过")
            
            # 可以选择提交到GitHub（如果文件有更新）
            # 这里可以选择性地进行git操作
            print("ℹ️ 文件质量良好，保持现有状态")
        else:
            print("❌ 现有文件质量检查失败，可能需要重新生成")
            # 可以选择重新生成或者标记为需要修复
            return False
...
def main():
    """主函数"""
    success = execute_hourly_digest_check()
    
    if success:
        print("✅ 每小时digest任务执行完成")
        sys.exit(0)
    else:
        print("❌ 每小时digest任务执行失败")
        sys.exit(1)
